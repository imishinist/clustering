...
template <typename T>
void calc_means(const std::vector<int>& belongs_to,
...
template <typename T>
bool assign_means(std::vector<int>& belongs_to,
...
template <typename T>
void initialize_kpp(std::vector<int>& belongs_to,
                    std::vector<std::vector<double>>& means,
...
std::vector<int> k_means(const std::vector<std::vector<T>>& points, int k) {
    ...
    std::vector<std::vector<double>> means(
        k, std::vector<double>(points[0].size(), 0));
