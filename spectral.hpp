...
matrix similarity_matrix(const std::vector<point>& points,
...
std::vector<int> ncut(const std::vector<point>& points, int k, double sigma2) {
    ...
    return spectral(decomposition(solve.eigenvalues(), solve.eigenvectors()),
                    k);
}
...
std::vector<int> just_k_means(const std::vector<point>& points, int k,
                              double sigma2) {
    int n = points.size();
